...
export class Cache<Key = GenericCacheKey, Value = unknown> {
  ...
  constructor(config?: CacheConfig<Key, Value>) {
    if (config) {
      ...
      this.#onDeleteExpired = config.onDeleteExpired
  ...
  #updateLRU(entry: CacheEntry<Key, Value>, evictLRU?: boolean): void {
    if (this.#lruMaxSize) {
  ...
  has(key: Key, options?: {
    ...
    if (entry) {
      const expired = options?.expired != null ? options?.expired : this.#getExpired;
      if (expired) {
        return true;
      }
      const now = this.now();
